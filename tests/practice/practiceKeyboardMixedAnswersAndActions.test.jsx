...
describe('User answers all phrases in a session correctly', () => {
  ...
  it('User performs the flow with the keyboard', async () => {
    ...
    expect(screen.getByText('Инструкция', {selector: 'button'})).toHaveFocus()

    await user.tab()

    expect(screen.getByText('Выход', {selector: 'button'})).toHaveFocus()

    await user.tab()

    expect(screen.getByTestId('remaining')).toHaveFocus()

    await user.tab()

    expect(screen.getByTestId('correct')).toHaveFocus()

    await user.tab()

    expect(screen.getByTestId('wrong')).toHaveFocus()

    await user.tab()

    expect(screen.getByTestId('all')).toHaveFocus()

    await user.tab()

    expect(screen.getByTestId('practice-phrase')).toHaveFocus()
    ...
    expect(screen.queryByText('Кто доктор?', {selector: '#practice span'})).not.toBeInTheDocument()
    expect(screen.queryByText('Фразы закончились! Начните сессию снова или поработайте с отдельными фразами.', {selector: '#practice span'})).not.toBeInTheDocument()
